...
def normalize_email(email):
    """Convertit le domaine d'un email Unicode en Punycode"""
    try:
...
def signup():
    if request.method == 'POST':
        ...
        if not re.match(r'^[a-zA-Z0-9_.-]{3,20}$', username):
            ...
            flash('Adresse e-mail invalide ou non compatible avec les noms de domaine internationalisés.')
        elif len(password) < 6:
            ...
            try:
                conn = sqlite3.connect('users.db')
                cursor = conn.cursor()
                cursor.execute('INSERT INTO users (username, email, password) VALUES (?, ?, ?)', (username, normalized_email, hashed_pw))
                conn.commit()
                conn.close()
...
def login():
    if request.method == 'POST':
        ...
        if normalized_email:
            cursor.execute('SELECT * FROM users WHERE username = ? OR email = ?', (identity, normalized_email))
        else:
            cursor.execute('SELECT * FROM users WHERE username = ?', (identity,))
